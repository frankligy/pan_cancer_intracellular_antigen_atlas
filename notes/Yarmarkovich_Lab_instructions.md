...
You can make sure all figures are generated by running below, it should output `done` and exit, if not, let it rescue these ones:

```bash
/gpfs/data/yarmarkovichlab/softwares/NeoVerse/launch_portal.py --config template.json --running_mode generate_figures_rescue
```
